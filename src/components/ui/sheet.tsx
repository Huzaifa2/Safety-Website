...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 gap-4 bg-card p-6 shadow-soft transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out",
        side === "right" &&
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 opacity-70 ring-offset-background transition hover:opacity-100 hover:bg-accent focus:outline-none">
        <X className="h-4 w-4" />
